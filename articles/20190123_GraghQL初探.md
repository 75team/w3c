...
![](https://p0.ssl.qhimg.com/t01e5f32f1d0b8cb976.png)

看上去不错。

#### 回到书作者的例子

我们考虑一个更实用的例子。回到上文说到的需求：

1. 需要获取某书的作者的详情
1. 对于每个作者，如果是 PC 端的请求，需要将作者的第一个相册的第一个照片取出来，对于移动端的需求，不需要取照片。

##### vue 程序

为了简化问题，我们依旧在 HelloWorld.vue 中进行修改。我们把之前的`gql`标签修改为使用单独的 schema 去调用单独 schema 文件。

针对查询类型，vue-apollo 库为我们提供了 ApolloQuery 组件。类似的还有 ApolloMutation 和 ApolloSubscription 组件。根据需求，这里使用 ApolloQuery 组件。

为了清晰，我们在 components 同级建立一个 graghql 的目录，所有的 query 和 mutation 都以`.gql`为扩展名放进去。

我们建立四个 Type：Book、Author、Alblum、Photo
